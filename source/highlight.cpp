...
static void int_highlight(State &st, const Expr *expr, std::string &top,
  ...
  if (auto v = dynamic_cast<const Var *>(expr); v) {
    ...
    int_highlight(st, a->arg, top, bot, /* combine: */ false,
    ...
    if (auto inner = dynamic_cast<Lambda *>(f->body);
      ...
      // we must break up the lambda so we don't end up with λx y x y. (...),
      // but rather λx y.λx y.( ... )
...
const static auto ALPHA_HIGHLIGHT =
    zpr::sprint("{}{}{}", GREEN_BOLD, UNDERLINE, COLOUR_RESET);
const static auto BETA_VAR_HIGHLIGHT =
    zpr::sprint("{}{}{}", YELLOW_BOLD, '^', COLOUR_RESET);
...
std::pair<std::string, std::string>
logAlphaConversion(const Expr **whole, const Expr *sub, int print_flags) {
  return highlight(
      *whole,
...
std::pair<std::string, std::string>
logBetaReduction(const Expr **whole, const Expr *fn, const Expr *arg,
                 const std::vector<Expr **> &_substs, int print_flags) {
  std::set<const Expr *> subs;
  std::transform(_substs.begin(), _substs.end(),
                 std::inserter(subs, subs.begin()), [](auto x) { return *x; });
...
std::string print(const ast::Expr *expr, int flags) {
  return highlight(
             expr, [](auto) { return std::nullopt; },
             [](auto) { return std::nullopt; }, flags)
      .first;
}

std::string
print(const ast::Expr *expr,
      std::function<std::optional<std::string>(const ast::Expr *)> replace,
      int flags) {
  State st{};
